...
public class Group implements Comparable, Cloneable { //убрать два последних слова
    ...
    public void addCadres(Cadres newCadres){
        ...
        } else System.out.println("Cadres " + newCadres.getName() + " already working in group: " + newCadres.getGroup().getGroupName());
    ...
    public void delCadres(String name){ //удаление по имени
        ...
        if(countSt == 0){
            System.out.println("No student " + name + " in "  + groupName + " group"); //если студентов нет
        }
        if(countSt == 1){ //если один студент, то удаляем
            for(int i = 0; i < groupList.length; i++){
                if(groupList[i]!=null && groupList[i].getName().equals(name)) {
                    ...
                    System.out.println("Student " + name + " is deleted from the group" + groupName);
                    break;
                }
            }
        }
        if(countSt > 1){ //если больше одного
            System.out.println("Student " + name + " in group " + groupName + " more then 1!"); //сообщаем что студентов больше 1
            searchCadr(name); // показываем нумерованный список студентов с такой фамилией
            System.out.println("Please use the method delStudent(int number)"); //просим удалить по номеру
        }
    }
    public void searchCadr(String name){ //поиск всех студентов с такой фамилией
        boolean isFound = false; //найден ли хотябы один студент с требуемой фамилией
        for(int i = 0; i < groupList.length; i++){
            if (groupList[i]!=null && groupList[i].getName().equals(name)) {
                System.out.println("Student " + name + " is found in group. List number is " + (i + 1) + ".");
                groupList[i].info();
                isFound = true;
            }
        }
        if(!isFound){
            System.out.println("No student " + name + " in "  + groupName + " group"); //если не найден
        }
    }
    ...
    public void delCadres(int number){ //удаляем  по номеру в группе
        try {
            String bufferCadrName = groupList[number-1].getName(); //запоминаем фамилию студента чтоб вывести сообщение после удаления
